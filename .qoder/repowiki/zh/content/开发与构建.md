...
- **调试功能增强**
  - 内存使用跟踪：在保存项目时统计信号数量、分隔符和沿标注数量，输出详细的内存使用信息
  - JSON序列化调试：在导出和保存时对比原始对象与序列化后的数据差异，监控序列化性能
  - 导入操作监控：在加载项目文件时统计分隔符和沿标注数量，验证导入数据完整性
...
+saveTemplate()
+importProject(file)
+loadImportedProject(data)
+loadLegacyProject(data)
}
class Exporter {
+exportPNG()
+exportSVG()
...
- 存储管理（src/io/StorageManager.js）
  ...
  - **增强的调试功能**：在导出项目时统计每个 sheet 中的信号数量和分隔符数量，输出详细的导出进度信息。
- 导出器（src/io/Exporter.js）
  - 支持 SVG/PNG/JSON 导出与剪贴板复制，提供独立 HTML 导出，内联样式与模块源码。
  - **增强的调试功能**：在复制到剪贴板时跟踪 SVG 序列化、图像加载、Canvas 绘制和 Blob 转换的完整流程，监控各阶段的性能指标。
...
- 导出性能
  ...
  - **增强的性能监控**：导出器在复制到剪贴板时跟踪 SVG 序列化长度、图像加载时间、Canvas 尺寸和 Blob 大小等关键指标。
- 存储与模板
  - 模板与项目数据尽量精简，避免不必要的字段。
  - 多 sheet 场景下，批量导入导出时一次性写入，减少多次 I/O。
  - **内存使用优化**：保存项目时统计信号数量、分隔符和沿标注数量，帮助识别内存使用模式。
- 构建体积
...
- 运行时异常
  ...
  - **调试增强**：利用增强的调试输出跟踪内存使用、JSON 序列化和导入操作的完整流程。
- 样式问题
...
- 严格遵循模块化与事件驱动的设计原则
- 重视测试与本地存储的兼容性与稳定性
- **充分利用增强的调试功能**：利用内存使用跟踪、JSON序列化调试和导入操作监控来优化性能和诊断问题
...
- **调试验证**
  - 利用增强的调试输出监控内存使用、JSON序列化和导入操作
  - 在开发过程中持续观察调试日志以识别性能瓶颈
...
- 导出 PNG 失败或空白
  ...
  - 确认 Canvas 尺寸与缩放系数
  - **性能监控**：查看导出器的调试输出，确认 SVG 序列化和图像加载阶段的性能指标
- 剪贴板复制失败
  - 检查浏览器权限与 Clipboard API 支持，必要时降级到 data URL 或新窗口方案
  - **调试追踪**：利用增强的调试输出跟踪复制过程的各个阶段
- 多 sheet 切换异常
  - 确认注册表与 sheet 数据一致性，检查事件监听与自动保存绑定
- **内存使用问题**
  - 观察保存项目时的内存使用跟踪输出，识别信号过多或数据冗余的情况
  - 检查分隔符和沿标注的数量，优化数据结构
...
- 代码审查
  - 关注模块职责、性能影响与兼容性
  - **调试功能审查**：确保新增的调试输出不会影响生产性能，并保持适当的日志级别