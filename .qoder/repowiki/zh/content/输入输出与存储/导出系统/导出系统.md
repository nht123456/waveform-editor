...
**更新** 本次更新重点关注独立HTML导出功能的改进，包括增强的调试能力、更好的错误处理机制、缓存管理优化和安全性提升。
...
- [main.js:1-1074](file://src/main.js#L1-L1074)
...
### 独立HTML导出功能
独立HTML导出是最复杂的功能，需要从源文件构建完整的可执行HTML文件。**更新** 该功能经过重大改进，增强了调试能力、错误处理和安全性。

#### HTML模板生成流程
```mermaid
flowchart TD
Start([开始导出独立HTML]) --> DebugInfo["输出调试信息<br/>箭头曲率检查"]
DebugInfo --> LoadTemplate["加载模板JSON"]
LoadTemplate --> LoadIndex["获取index.html源码<br/>使用时间戳参数避免缓存"]
LoadIndex --> LoadCSS["获取CSS样式<br/>错误处理和日志"]
LoadCSS --> LoadModules["按依赖顺序加载JS模块<br/>逐个模块错误处理"]
LoadModules --> ProcessModules["处理模块代码<br/>移除import/export<br/>转义</script>标签"]
ProcessModules --> InlineCSS["内联CSS样式"]
InlineCSS --> InlineJS["内联JavaScript代码"]
InlineJS --> ReplaceTemplate["替换模板变量<br/>HTML转义防止注入"]
...
**图表来源**
- [Exporter.js:200-306](file://src/io/Exporter.js#L200-L306)
...
5. **缓存管理**：使用时间戳参数避免浏览器缓存问题
6. **安全防护**：对模板JSON进行HTML转义，防止脚本注入攻击

**更新** 新增的调试功能会在导出过程中输出详细的诊断信息，包括箭头曲率检查和项目数据统计。
...
2. 验证模块依赖关系
3. 确认代码转换过程无错误
4. 确保通过HTTP服务器访问，避免file://协议限制
5. 检查控制台是否有详细的错误信息

**更新** 独立HTML导出功能现在提供详细的调试信息，包括箭头曲率检查和项目数据统计，有助于快速定位问题。
...
6. **调试增强**：独立HTML导出功能具备详细的诊断信息输出
7. **安全性提升**：对模板数据进行HTML转义，防止脚本注入攻击
8. **缓存管理**：使用时间戳参数避免浏览器缓存问题
...
4. **缓存问题**：如果遇到缓存问题，刷新页面或清除浏览器缓存
5. **调试信息**：利用控制台输出的调试信息排查问题
...
5. **模板数据安全检查**：对嵌入的模板数据进行HTML转义处理

**更新** 独立HTML导出功能新增了箭头曲率检查和项目数据统计功能，提供更全面的数据验证能力。
