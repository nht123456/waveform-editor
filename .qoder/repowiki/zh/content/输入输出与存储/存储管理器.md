...
## 更新摘要
**变更内容**
- 更新了非破坏性导入功能的描述，强调loadImportedProject方法现在使用非破坏性方式导入项目
- 新增了loadImportedProject方法的详细行为说明，包括新ID生成和注册表追加机制
- 更新了项目文件导入导出功能的架构说明，反映了非破坏性导入的实现原理
- 增强了错误处理和数据安全性的说明
...
**更新** 增强了导出功能的调试能力，在exportProject方法中添加了详细的信号计数和分隔符计数日志，便于开发和调试过程中的性能分析和数据验证。同时，loadImportedProject方法现在提供非破坏性导入功能，确保现有数据不会被覆盖，而是生成新ID并添加到注册表中。
...
- [StorageManager.js:346-380](file://src/io/StorageManager.js#L346-L380)
...
+loadImportedProject(data) string
...
**更新** 导入功能现在提供非破坏性导入机制，确保现有数据不会被覆盖：
...
participant SM as "存储管理器"
participant LS as "localStorage"
UI->>SM : importProject(file)
SM->>SM : 读取并解析文件
alt 旧版格式
SM-->>UI : 返回{legacy : true, project : data}
else 新版格式
SM-->>UI : 返回{legacy : false, registry : data.registry, sheets : data.sheets}
end
UI->>SM : loadImportedProject(result)
SM->>SM : loadRegistry() 获取当前注册表
SM->>SM : 为每个导入的sheet生成新ID
SM->>LS : saveSheet(新ID, sheetData)
SM->>SM : 追加到注册表
SM->>SM : 设置第一个导入的sheet为活跃状态
SM->>SM : 保存更新后的注册表
SM-->>UI : 返回第一个导入的sheetId
```

**图表来源**
- [StorageManager.js:210-268](file://src/io/StorageManager.js#L210-L268)

导入功能支持多种文件格式：
- **新版格式(.wfp)**：包含完整的注册表和工作表数据，支持非破坏性导入
- **旧版格式(.json)**：兼容原有的单项目格式，使用loadLegacyProject处理
- **格式验证**：自动检测并处理不同的文件格式

**章节来源**
- [StorageManager.js:168-285](file://src/io/StorageManager.js#L168-L285)
...
**更新** 非破坏性导入功能在处理大量工作表时具有良好的性能表现，通过批量ID生成和注册表追加操作避免了重复的存储写入操作。
...
**更新** 如果遇到非破坏性导入相关的问题：
- 确认导入的项目文件格式正确（.wfp格式）
- 检查导入的项目是否包含有效的注册表和工作表数据
- 验证现有工作表ID冲突问题，确保新ID生成机制正常工作
- 查看控制台日志确认导入过程中的ID映射和注册表更新
...
**更新** 最重要的改进是loadImportedProject方法现在提供非破坏性导入功能，确保用户可以在不覆盖现有数据的情况下安全地导入新项目。这一功能通过生成新ID、追加到注册表和保持原有活跃工作表不变的机制，大大提高了数据安全性。
...
```javascript
// 导入项目文件
const result = await storageManager.importProject(file);

if (result.legacy) {
  // 处理旧版格式
  storageManager.loadLegacyProject(result.project);
} else {
  // 非破坏性导入新格式
  const firstSheetId = storageManager.loadImportedProject(result);
  console.log('导入完成，第一个新工作表ID:', firstSheetId);
}
...
**更新** loadImportedProject方法的非破坏性导入机制：
...
**非破坏性导入的关键特性**：
- **ID生成**：为每个导入的工作表生成新的随机ID，确保与现有ID不冲突
- **数据复制**：将导入的工作表数据复制到新的ID下，不修改原始数据
- **注册表追加**：将新工作表信息追加到现有注册表末尾
- **活跃状态保持**：导入后保持原有的活跃工作表不变
- **返回值**：返回第一个导入工作表的ID，便于UI切换

这种设计确保了用户可以安全地导入多个项目而不影响现有的工作表，提供了更好的数据管理和组织能力。