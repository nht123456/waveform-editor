...
**变更内容**
- 新增常量波形检测回退机制的详细说明
- 更新图像识别算法流程图以反映常量检测步骤
- 增强信号分析章节以包含常值信号处理
- 完善故障排除指南中的常量信号识别问题
- 新增ImageRecognizer.js模块的详细实现分析
...
- **常量波形检测回退机制**：当主要分析无法生成信号段时，系统能够自动检测常值信号（全0或全1）
...
**图表来源**
- [src/main.js:1-1074](file://src/main.js#L1-L1074)
...
6. **常量信号检测回退**：当主要分析失败时，通过像素密度分布检测常值信号
7. **OCR可选识别**：识别信号名称和总线值
...
ImageRecognizer实现了完整的波形图像识别流程，包括增强的常量波形检测回退机制：
...
BusLane --> SegmentCheck{能生成段?}
NormalLane --> SegmentCheck
SegmentCheck --> |是| ClockPattern{时钟模式?}
SegmentCheck --> |否| ConstantFallback[常量检测回退]
ConstantFallback --> ConstantLevel[检测常值电平]
ConstantLevel --> CreateConstantSignal[创建常量信号]
ClockPattern --> |是| CreateClock[创建时钟信号]
ClockPattern --> |否| CreateNormal[创建普通信号]
CreateClock --> OCRCheck{OCR启用?}
CreateConstantSignal --> OCRCheck
...
- **常量检测回退**：当主要分析失败时，通过像素密度分布检测常值信号

### 常量波形检测回退机制

系统新增了智能的常量波形检测回退机制，能够在主要分析无法生成有效信号段时自动检测常值信号：

**检测原理：**
- 通过分析波形区域上下半部分的像素密度分布
- 上半部分像素数多 → 高电平线在上方 → 常1
- 下半部分像素数多 → 低电平线在下方 → 常0
- 采样步长优化：最多100个采样点平衡精度和性能

**实现细节：**
- 采样步长计算：`step = Math.max(1, Math.floor((xEnd - xStart) / 100))`
- 像素计数：分别统计上半和下半区域的前景像素
- 电平判定：`return upperCount > lowerCount ? 1 : 0`
...
ConstantSample[常量检测采样] --> Size6[最多100个点 × 2字节]
Memory --> Canvas
Memory --> ImageData
Memory --> Binary
Memory --> Signals
Memory --> Segments
Memory --> ConstantSample
```
...
**常量信号识别问题：**
- **症状**：常值信号（全0或全1）无法正确识别
- **原因**：信号线过细或对比度不足
- **解决方案**：系统已内置常量检测回退机制，可自动识别常值信号
- **建议**：确保图像清晰度足够，信号线宽度至少2像素
...
- **常量波形检测回退机制**：当主要分析失败时，系统能够自动检测常值信号，显著提升识别成功率
...
该系统为未来的扩展奠定了良好的基础，可以进一步集成更高级的机器学习算法，支持更多类型的波形识别，以及提供更丰富的编辑和分析功能。常量波形检测回退机制的引入使得系统在面对复杂或低质量图像时更加稳健可靠，大大提升了实际应用中的识别成功率。

**更新** 本版本特别增强了对ImageRecognizer.js模块的分析，详细介绍了其常量波形检测回退机制的实现原理和应用场景，为系统的技术深度提供了重要补充。