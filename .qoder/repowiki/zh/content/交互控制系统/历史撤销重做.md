...
**更新** 本次更新反映了新增的 edgeMarkers 历史记录类型，以及增强的 deleteGap、addArrow、deleteArrow 等操作的历史跟踪功能，完善了撤销重做系统的功能覆盖。
...
**更新** 新增 edgeMarkers 历史记录类型，支持跳变沿标注的撤销重做；增强的 deleteGap、addArrow、deleteArrow 操作提供了更完整的历史跟踪。
...
- 性能问题
  ...
  - 评估命令对象快照大小，避免不必要的字段。
- **新增** edgeMarkers 相关问题
  - 确认 Signal.edgeMarkers 数组的正确初始化和序列化。
  - 检查 SignalRenderer 是否正确渲染 edgeMarkers 标注。
  - 验证标注类型（rising/falling）的区分和渲染逻辑。
...
**更新** 本次更新显著增强了系统的功能覆盖，新增的 edgeMarkers 历史记录类型和增强的 deleteGap、addArrow、deleteArrow 操作历史跟踪，使得撤销重做功能更加完善和实用。开发者可在现有框架上扩展更多命令类型、优化快照策略，并根据业务需求调整历史数量上限与 UI 交互细节。
