# 依赖箭头管理

<cite>
**本文档引用的文件**
- [src/models/Arrow.js](file://src/models/Arrow.js)
- [src/models/Project.js](file://src/models/Project.js)
- [src/renderers/DependencyRenderer.js](file://src/renderers/DependencyRenderer.js)
- [src/renderers/SVGRenderer.js](file://src/renderers/SVGRenderer.js)
- [src/controllers/InteractionController.js](file://src/controllers/InteractionController.js)
- [src/config/colors.js](file://src/config/colors.js)
- [src/main.js](file://src/main.js)
- [src/ui/PropertyPanel.js](file://src/ui/PropertyPanel.js)
</cite>

## 更新摘要
**变更内容**
- 优化了同信号弧形箭头的控制点计算算法，改进了不对称控制点位置计算
- 解决了S形弯曲视觉伪影问题，提升了同信号自连接箭头的显示质量
- 增强了弧高计算和控制点定位的实现细节，支持曲率缩放参数

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)

## 简介

依赖箭头管理系统是波形图编辑器的核心功能模块之一，负责管理信号之间的依赖关系。该系统提供了完整的箭头创建、编辑、渲染和交互功能，包括Alt键触发机制、临时箭头渲染、箭头吸附功能、端点拖拽、文字标签拖拽和箭头删除操作。

**更新** 本次更新优化了同信号弧形箭头的控制点计算算法，改进了不对称控制点位置计算，解决了S形弯曲视觉伪影问题，显著提升了同信号自连接箭头的视觉效果和用户体验。

## 项目结构

该项目采用模块化架构设计，主要包含以下核心目录和文件：

```mermaid
graph TB
subgraph "核心模型层"
Arrow[Arrow.js<br/>箭头模型]
Project[Project.js<br/>项目模型]
Signal[Signal.js<br/>信号模型]
Segment[Segment.js<br/>波形段模型]
end
subgraph "渲染层"
SVGRenderer[SVGRenderer.js<br/>SVG渲染器]
DependencyRenderer[DependencyRenderer.js<br/>依赖箭头渲染器]
SignalRenderer[SignalRenderer.js<br/>信号渲染器]
TimeAxisRenderer[TimeAxisRenderer.js<br/>时间轴渲染器]
end
subgraph "控制器层"
InteractionController[InteractionController.js<br/>交互控制器]
HistoryController[HistoryController.js<br/>历史控制器]
end
subgraph "用户界面"
PropertyPanel[PropertyPanel.js<br/>属性面板]
SignalPanel[SignalPanel.js<br/>信号面板]
Toolbar[Toolbar.js<br/>工具栏]
end
subgraph "配置和工具"
Colors[colors.js<br/>颜色配置]
StorageManager[StorageManager.js<br/>存储管理器]
Exporter[Exporter.js<br/>导出器]
end
Arrow --> Project
Project --> SVGRenderer
SVGRenderer --> DependencyRenderer
SVGRenderer --> SignalRenderer
SVGRenderer --> TimeAxisRenderer
InteractionController --> Project
InteractionController --> SVGRenderer
PropertyPanel --> Project
Colors --> SVGRenderer
Colors --> DependencyRenderer
```

**图表来源**
- [src/models/Arrow.js:1-122](file://src/models/Arrow.js#L1-L122)
- [src/renderers/DependencyRenderer.js:1-306](file://src/renderers/DependencyRenderer.js#L1-L306)
- [src/controllers/InteractionController.js:1-1579](file://src/controllers/InteractionController.js#L1-L1579)

**章节来源**
- [src/main.js:1-819](file://src/main.js#L1-L819)

## 核心组件

### 箭头模型 (Arrow)

Arrow类是依赖箭头的核心数据模型，负责表示信号之间的依赖关系。其主要特性包括：

- **唯一标识符**: 自动生成的唯一ID
- **端点信息**: 起点和终点的信号ID及时间坐标
- **控制点**: 用于贝塞尔曲线的控制点偏移
- **方向属性**: 支持自动、正向、反向三种模式
- **双向箭头**: 支持双向箭头显示
- **标签系统**: 支持多个文字标签，每个标签包含文本内容和偏移量
- **样式配置**: 支持颜色、线宽、箭头大小、虚线样式等
- **曲线类型**: 支持弧线(curved)和直线(straight)两种模式
- **曲率参数**: 支持0.2-3.0范围的曲率缩放，用于控制弧形程度

### 项目模型 (Project)

Project类管理整个波形图项目的数据结构，包含：

- **信号集合**: 管理所有信号对象
- **箭头集合**: 管理所有依赖箭头
- **时间轴配置**: 管理时间轴的起始、结束时间和缩放比例
- **事件系统**: 提供change事件通知机制

### 依赖箭头渲染器 (DependencyRenderer)

DependencyRenderer专门负责渲染依赖箭头，具有以下功能：

- **贝塞尔曲线计算**: 生成平滑的S形曲线连接两个信号
- **防重叠处理**: 同一起点或多箭头时的垂直偏移处理
- **双向箭头支持**: 根据配置显示正向或双向箭头
- **命中区域**: 透明的命中区域便于选择和拖拽
- **标签渲染**: 支持多个标签的渲染和交互
- **同信号弧形渲染**: 特殊处理同信号箭头的向上拱起弧线
- **曲线类型支持**: 支持弧线和直线两种曲线模式
- **曲率缩放**: 通过curvature参数控制曲线的弯曲程度

**更新** 优化了同信号弧形箭头的控制点计算算法，改进了不对称控制点位置计算，解决了S形弯曲视觉伪影问题。

**章节来源**
- [src/models/Arrow.js:1-122](file://src/models/Arrow.js#L1-L122)
- [src/models/Project.js:1-245](file://src/models/Project.js#L1-L245)
- [src/renderers/DependencyRenderer.js:1-306](file://src/renderers/DependencyRenderer.js#L1-L306)

## 架构概览

依赖箭头管理系统采用分层架构设计，各层职责明确：

```mermaid
sequenceDiagram
participant User as 用户
participant IC as 交互控制器
participant DR as 依赖箭头渲染器
participant PRJ as 项目模型
participant SVGR as SVG渲染器
User->>IC : Alt+鼠标点击创建箭头
IC->>IC : _startArrowDrag()
IC->>DR : _renderTempArrow()
DR->>SVGR : 创建临时箭头SVG元素
User->>IC : 移动鼠标
IC->>DR : _renderTempArrow()
DR->>SVGR : 更新临时箭头位置
User->>IC : 松开鼠标
IC->>IC : _completeArrowCreation()
IC->>PRJ : addArrow()
PRJ->>SVGR : emit('change')
SVGR->>DR : render()
DR->>SVGR : 渲染最终箭头
```

**图表来源**
- [src/controllers/InteractionController.js:581-779](file://src/controllers/InteractionController.js#L581-L779)
- [src/renderers/DependencyRenderer.js:18-84](file://src/renderers/DependencyRenderer.js#L18-L84)

系统的关键交互流程包括：

1. **Alt键触发机制**: 检测Alt键按下状态，切换到箭头创建模式
2. **临时箭头渲染**: 实时渲染跟随鼠标的临时箭头
3. **吸附功能**: 自动吸附到目标信号的最近跳变沿
4. **箭头创建完成**: 将临时箭头转换为永久箭头并添加到项目中

**更新** 优化了同信号弧形箭头的控制点计算算法，在控制点计算阶段区分同信号和跨信号箭头的不同渲染策略，并引入了曲率缩放参数。

## 详细组件分析

### Alt键触发机制

Alt键触发机制是箭头创建流程的核心入口：

```mermaid
flowchart TD
Start([鼠标按下]) --> CheckAlt{"检查Alt键状态"}
CheckAlt --> |按下| StartDrag["_startArrowDrag()"]
CheckAlt --> |未按下| NormalSelect["正常信号选择"]
StartDrag --> RenderTemp["_renderTempArrow()"]
RenderTemp --> MouseMove["鼠标移动事件"]
MouseMove --> UpdateTemp["_renderTempArrow()"]
UpdateTemp --> MouseUp["鼠标松开事件"]
MouseUp --> Complete["_completeArrowCreation()"]
Complete --> AddArrow["Project.addArrow()"]
AddArrow --> RenderFinal["DependencyRenderer.render()"]
NormalSelect --> End([结束])
RenderFinal --> End
```

**图表来源**
- [src/controllers/InteractionController.js:177-181](file://src/controllers/InteractionController.js#L177-L181)
- [src/controllers/InteractionController.js:581-591](file://src/controllers/InteractionController.js#L581-L591)

### 优化的同信号弧形箭头控制点计算

**更新** 本次更新的核心是优化了同信号弧形箭头的控制点计算算法：

```mermaid
flowchart TD
MouseMove["鼠标移动"] --> CheckSignal{"检查信号一致性"}
CheckSignal --> |同一信号| ArcCalc["计算弧形控制点"]
CheckSignal --> |不同信号| SCurve["计算S形控制点"]
ArcCalc --> Curvature["应用曲率缩放参数"]
Curvature --> ArcHeight["弧高 = max(35, min(dx * 0.35, 80)) * curvature"]
ArcHeight --> AsymmetricCP["计算不对称控制点"]
AsymmetricCP --> ControlPoints["cp1: dx * 0.3, cp2: dx * 0.25"]
ControlPoints --> RenderPath["渲染贝塞尔曲线"]
SCurve --> ControlPoints
RenderPath --> UpdateArrow["更新箭头路径"]
```

**图表来源**
- [src/renderers/DependencyRenderer.js:289-297](file://src/renderers/DependencyRenderer.js#L289-L297)
- [src/controllers/InteractionController.js:719-728](file://src/controllers/InteractionController.js#L719-L728)

#### 优化的控制点计算算法

同信号箭头的控制点计算遵循以下优化规则：

1. **弧高计算**: `arcHeight = Math.max(35, Math.min(dx * 0.35, 80)) * curvature`
   - 最小弧高：35像素
   - 最大弧高：80像素
   - 动态弧高：根据水平距离的35%计算
   - 曲率缩放：通过curvature参数控制弧形程度

2. **不对称控制点位置**:
   - `cp1.x = fromX + dx * 0.3 * direction`
   - `cp1.y = fromY - arcHeight`
   - `cp2.x = toX - dx * 0.25 * direction`
   - `cp2.y = toY - arcHeight * 0.15`

3. **关键优化**:
   - **cp1和cp2使用不同的水平偏移比例**：0.3 vs 0.25，实现不对称控制点
   - **cp2的垂直位置更接近终点**：`arcHeight * 0.15`，解决S形弯曲视觉伪影
   - **曲率参数支持**：0.2-3.0范围，允许用户调整弧形程度

#### 视觉效果增强

优化后的同信号弧形箭头相比之前的版本具有以下优势：

- **更好的视觉层次**: 向上拱起的弧线更加突出
- **更强的识别度**: 自连接箭头更容易被识别
- **更佳的用户体验**: 符合用户对自连接关系的预期
- **统一的视觉风格**: 与其他UI元素保持一致的曲线风格
- **解决视觉伪影**: 通过不对称控制点和调整的垂直位置消除了S形弯曲的视觉问题

**章节来源**
- [src/renderers/DependencyRenderer.js:289-306](file://src/renderers/DependencyRenderer.js#L289-L306)
- [src/controllers/InteractionController.js:719-741](file://src/controllers/InteractionController.js#L719-L741)

### 临时箭头渲染系统

临时箭头渲染系统提供了实时的视觉反馈：

| 功能特性 | 实现方式 | 参数说明 |
|---------|----------|----------|
| 贝塞尔曲线控制点 | `Math.min(dx * 0.5, 150)` | 控制曲线弯曲程度 |
| 同信号弧形控制点 | `Math.max(35, Math.min(dx * 0.35, 80))` | 向上拱起的弧线高度 |
| 箭头吸附 | `snapToEdge()`方法 | 自动对齐到信号跳变沿 |
| 透明度效果 | `rgba(0, 120, 215, 0.6)` | 半透明显示临时箭头 |
| 虚线样式 | `stroke-dasharray: '5,3'` | 区分临时和永久箭头 |

**更新** 新增了同信号弧形箭头的特殊控制点计算，当起点和终点在同一信号线上时，会生成向上拱起的弧线，弧高范围为35-80像素之间。

### 箭头吸附功能

吸附功能确保箭头端点能够精确对齐到目标信号：

```mermaid
flowchart TD
MouseMove["鼠标移动"] --> GetSignal["获取信号索引"]
GetSignal --> HasSignal{"目标信号存在?"}
HasSignal --> |是| SnapToEdge["调用snapToEdge()"]
HasSignal --> |否| UseRaw["使用原始坐标"]
SnapToEdge --> CalcTime["计算时间坐标"]
CalcTime --> CalcXY["转换为X,Y坐标"]
CalcXY --> UpdatePreview["更新预览位置"]
UseRaw --> UpdatePreview
UpdatePreview --> StoreValues["存储吸附值"]
```

**图表来源**
- [src/controllers/InteractionController.js:698-714](file://src/controllers/InteractionController.js#L698-L714)
- [src/controllers/InteractionController.js:257-282](file://src/controllers/InteractionController.js#L257-L282)

### 箭头编辑功能

系统提供了完整的箭头编辑功能：

#### 端点拖拽编辑

端点拖拽功能允许用户重新定位箭头的起点或终点：

| 拖拽类型 | 触发条件 | 预览效果 | 数据更新 |
|---------|----------|----------|----------|
| 起点拖拽 | 点击起点圆圈 | 虚线预览圆圈 | 更新`fromTime`和`fromSignalId` |
| 终点拖拽 | 点击终点圆圈 | 虚线预览圆圈 | 更新`toTime`和`toSignalId` |
| 边界检查 | 拖拽过程 | 防止起点终点相同 | 验证数据有效性 |

#### 文字标签拖拽

文字标签支持独立的拖拽编辑：

```mermaid
sequenceDiagram
participant User as 用户
participant IC as 交互控制器
participant Arrow as 箭头模型
participant Renderer as 渲染器
User->>IC : 点击标签文字
IC->>IC : _startArrowTextDrag()
IC->>IC : 记录初始偏移量
User->>IC : 拖拽标签
IC->>IC : _updateArrowTextDrag()
IC->>Arrow : 更新标签偏移
IC->>Renderer : renderer.render()
User->>IC : 松开鼠标
IC->>IC : 完成拖拽操作
```

**图表来源**
- [src/controllers/InteractionController.js:596-629](file://src/controllers/InteractionController.js#L596-L629)

#### 箭头删除操作

删除操作通过键盘快捷键实现：

| 操作方式 | 触发条件 | 行为描述 |
|---------|----------|----------|
| Delete键 | 选中箭头时 | 删除当前选中的箭头 |
| 双击标签 | 双击箭头标签 | 选中箭头并打开属性面板 |
| 双击箭头 | 双击箭头主体 | 添加新的标签 |

### 箭头选择机制和视觉反馈

选择机制提供了清晰的视觉反馈：

```mermaid
stateDiagram-v2
[*] --> Unselected
Unselected --> Selected : 点击箭头
Selected --> Editing : 开始编辑
Editing --> Selected : 编辑完成
Selected --> Unselected : 点击空白处
Editing --> Unselected : 清除选择
state Selected {
[*] --> GlowEffect
GlowEffect --> Highlight : 选中状态
Highlight --> GlowEffect : 预览效果
}
```

**图表来源**
- [src/renderers/DependencyRenderer.js:148-162](file://src/renderers/DependencyRenderer.js#L148-L162)
- [src/controllers/InteractionController.js:1348-1355](file://src/controllers/InteractionController.js#L1348-L1355)

### 箭头标签管理系统

标签系统支持多标签管理：

| 标签操作 | 方法 | 功能描述 |
|---------|------|----------|
| 添加标签 | `addLabel(text, offset)` | 添加新的标签到箭头 |
| 删除标签 | `removeLabel(labelId)` | 从箭头移除指定标签 |
| 查找标签 | `getLabelById(labelId)` | 根据ID获取标签对象 |
| 文本访问 | `text` getter/setter | 兼容旧版本的文本访问 |
| 偏移访问 | `textOffset` getter/setter | 标签位置偏移管理 |

### 箭头属性控制系统

**新增功能** 箭头属性控制系统提供了对曲线类型和曲率的精细控制：

```mermaid
flowchart TD
PropertyPanel["属性面板"] --> CurveType["曲线类型选择"]
CurveType --> StraightMode["直线模式"]
CurveType --> CurvedMode["弧线模式"]
StraightMode --> StraightCP["直线控制点计算"]
CurvedMode --> CurvatureSlider["曲率滑块"]
CurvatureSlider --> CurvatureVal["曲率值显示"]
CurvatureVal --> ArcCalc["弧形控制点计算"]
ArcCalc --> AsymmetricCP["不对称控制点"]
AsymmetricCP --> RenderCurve["渲染优化曲线"]
```

**图表来源**
- [src/ui/PropertyPanel.js:326-338](file://src/ui/PropertyPanel.js#L326-L338)
- [src/models/Arrow.js:18-22](file://src/models/Arrow.js#L18-L22)

#### 曲线类型和曲率控制

箭头属性系统支持以下控制选项：

1. **曲线类型**:
   - `curved`: 默认弧线模式
   - `straight`: 直线模式

2. **曲率参数**:
   - 范围: 0.2 - 3.0
   - 默认值: 1.0
   - 影响: 控制弧形的弯曲程度

3. **属性面板集成**:
   - 曲线类型选择下拉菜单
   - 曲率滑块控件
   - 实时曲率值显示

**章节来源**
- [src/models/Arrow.js:18-22](file://src/models/Arrow.js#L18-L22)
- [src/ui/PropertyPanel.js:326-338](file://src/ui/PropertyPanel.js#L326-L338)

## 依赖关系分析

依赖箭头管理系统的核心依赖关系如下：

```mermaid
graph TB
subgraph "数据层"
ArrowModel[Arrow模型]
ProjectModel[Project模型]
SignalModel[Signal模型]
end
subgraph "渲染层"
SVGRenderer[SVG渲染器]
DependencyRenderer[依赖箭头渲染器]
SignalRenderer[信号渲染器]
TimeAxisRenderer[时间轴渲染器]
end
subgraph "控制层"
InteractionController[交互控制器]
HistoryController[历史控制器]
end
subgraph "界面层"
PropertyPanel[属性面板]
SignalPanel[信号面板]
Toolbar[工具栏]
end
subgraph "配置层"
Colors[颜色配置]
end
ArrowModel --> ProjectModel
ProjectModel --> SVGRenderer
SVGRenderer --> DependencyRenderer
SVGRenderer --> SignalRenderer
SVGRenderer --> TimeAxisRenderer
InteractionController --> ProjectModel
InteractionController --> SVGRenderer
PropertyPanel --> ProjectModel
HistoryController --> ProjectModel
```

**图表来源**
- [src/main.js:1-819](file://src/main.js#L1-L819)
- [src/renderers/SVGRenderer.js:1-200](file://src/renderers/SVGRenderer.js#L1-L200)

### 组件耦合度分析

系统采用松耦合设计，各组件间通过清晰的接口进行通信：

- **低耦合**: 渲染器与控制器分离，通过事件系统通信
- **高内聚**: 每个组件专注于特定功能领域
- **可扩展性**: 新增功能可通过扩展接口实现

### 外部依赖集成

系统集成了多种外部功能：

- **颜色配置**: 通过colors.js集中管理所有颜色
- **事件系统**: 基于标准DOM事件和自定义事件
- **本地存储**: 支持项目数据的持久化存储

**章节来源**
- [src/config/colors.js:1-83](file://src/config/colors.js#L1-L83)
- [src/renderers/SVGRenderer.js:1-200](file://src/renderers/SVGRenderer.js#L1-L200)

## 性能考虑

依赖箭头管理系统在性能方面采用了多项优化策略：

### 渲染优化

1. **分层渲染**: 将不同类型的元素渲染到不同的SVG组中
2. **增量更新**: 仅更新发生变化的部分，而非整幅画面
3. **虚拟DOM**: 使用createElement创建SVG元素，提高渲染效率
4. **弧形计算优化**: 同信号箭头使用快速的弧形计算算法
5. **曲率参数缓存**: 通过curvature参数避免重复计算

### 内存管理

1. **对象池**: 复用SVG元素，减少DOM操作
2. **事件清理**: 及时移除不再使用的事件监听器
3. **垃圾回收**: 合理管理临时对象的生命周期

### 交互响应

1. **请求动画帧**: 使用requestAnimationFrame优化动画性能
2. **节流处理**: 对频繁触发的事件进行节流处理
3. **延迟渲染**: 对复杂的渲染操作进行延迟执行

### 优化的同信号弧形渲染性能

**新增优化** 优化后的同信号弧形箭头渲染在性能方面的改进：

- **快速判断**: 使用`Math.abs(y2 - y1) < 5`快速判断同信号情况
- **简化计算**: 不对称控制点计算比S形曲线更简单
- **曲率缓存**: 通过curvature参数实现计算结果缓存
- **视觉伪影消除**: 通过调整控制点位置提升渲染质量

## 故障排除指南

### 常见问题及解决方案

#### 箭头无法创建

**问题症状**: Alt+点击无反应

**可能原因**:
1. 事件监听器未正确绑定
2. 信号索引计算错误
3. 项目数据状态异常

**解决步骤**:
1. 检查`_setupEventListeners()`方法是否执行
2. 验证`getSignalIndexByY()`方法的返回值
3. 确认项目状态的完整性

#### 箭头吸附不准确

**问题症状**: 箭头端点无法吸附到目标信号

**可能原因**:
1. 信号跳变沿检测算法问题
2. 坐标转换计算错误
3. 信号索引映射异常

**解决步骤**:
1. 检查`snapToEdge()`方法的实现
2. 验证坐标转换函数的准确性
3. 确认信号索引的正确性

#### 同信号弧形渲染异常

**问题症状**: 同信号箭头没有显示为弧形

**可能原因**:
1. 同信号判断逻辑错误
2. 控制点计算公式异常
3. SVG路径生成错误
4. **更新**: 曲率参数设置不当

**解决步骤**:
1. 检查`Math.abs(y2 - y1) < 5`判断条件
2. 验证弧高计算公式：`Math.max(35, Math.min(dx * 0.35, 80)) * curvature`
3. 确认控制点坐标计算的正确性
4. **更新**: 检查curvature参数是否在0.2-3.0范围内

#### 控制点不对称问题

**问题症状**: 同信号箭头的控制点位置不合理

**可能原因**:
1. **更新**: 控制点计算公式错误
2. **更新**: 不对称比例设置不当
3. **更新**: 曲率参数影响控制点位置

**解决步骤**:
1. **更新**: 检查cp1使用0.3比例，cp2使用0.25比例的计算
2. **更新**: 验证cp2垂直位置为arcHeight * 0.15的计算
3. **更新**: 确认曲率参数对控制点位置的影响

#### 渲染性能问题

**问题症状**: 箭头渲染卡顿或延迟

**可能原因**:
1. SVG元素过多导致的渲染压力
2. 事件处理函数执行时间过长
3. 重复渲染操作
4. **更新**: 优化的控制点计算算法

**解决步骤**:
1. 优化SVG元素的创建和复用
2. 减少事件处理函数的计算量
3. 实施渲染缓存机制
4. **更新**: 利用优化的控制点计算算法提升性能

**章节来源**
- [src/controllers/InteractionController.js:84-82](file://src/controllers/InteractionController.js#L84-L82)
- [src/renderers/DependencyRenderer.js:18-84](file://src/renderers/DependencyRenderer.js#L18-L84)

## 结论

依赖箭头管理系统是一个功能完整、架构清晰的模块化系统。它通过合理的分层设计、完善的事件处理机制和高效的渲染策略，为用户提供了一个流畅的箭头创建和编辑体验。

**更新** 本次更新优化了同信号弧形箭头的控制点计算算法，显著提升了系统的视觉效果和用户体验。通过改进不对称控制点位置计算、解决S形弯曲视觉伪影问题、引入曲率缩放参数，系统为同一信号线上的自连接箭头提供了更加美观、直观和可控的显示方式。

系统的主要优势包括：

1. **直观的交互设计**: Alt键触发机制符合用户预期
2. **精确的吸附功能**: 确保箭头端点能够准确定位
3. **丰富的编辑能力**: 支持端点拖拽、标签编辑等多种操作
4. **优秀的视觉效果**: 优化后的同信号弧形箭头提供了更好的视觉层次
5. **灵活的参数控制**: 通过curveType和curvature参数实现精细的曲线控制
6. **良好的性能表现**: 通过多种优化策略保证系统响应速度
7. **可扩展的架构**: 清晰的模块划分便于功能扩展和维护

未来可以考虑的改进方向：
- 增加更多曲线样式的自定义选项
- 实现箭头批量操作支持
- 优化大型项目的数据处理性能
- 添加箭头关系验证和约束检查
- 扩展弧形箭头的动画效果
- **更新**: 进一步优化控制点计算算法的性能
- **更新**: 增强曲率参数的可视化调节功能