...
- 时钟信号的特殊属性（period、dutyCycle、phase）与时钟周期计算逻辑
...
- 渲染层：SignalRenderer 基于 Project 和 Segment 数据进行可视化
- 输入输出层：StorageManager/Exporter 负责持久化与导出
- UI 层：SignalPanel/Toolbar 等提供用户交互
...
- 基本属性管理：id、name、type、color、segments、gaps、clockConfig
- 段管理：addSegment、setValueAt、getValueAt、getSegmentIndexAt、moveEdge
- 时钟信号：generateClockSegments、clockConfig（period、phase、dutyCycle）
...
- 时钟周期计算：generateClockSegments 按照 period、phase、dutyCycle 生成交替高低电平段
...
```mermaid
flowchart TD
Start(["开始 generateClockSegments"]) --> Init["初始化 time=phase, isHigh=dutyCycle>0"]
Init --> Loop{"time < endTime?"}
Loop --> |否| End(["结束"])
Loop --> |是| CalcDuration["计算本次持续时间<br/>高电平: period*dutyCycle<br/>低电平: period*(1-dutyCycle)"]
CalcDuration --> CreateSeg["创建段(startTime=time, endTime=min(time+duration, endTime), value=isHigh?1:0)"]
...
**图表来源**
- [Signal.js:43-56](file://src/models/Signal.js#L43-L56)
...
Project-->>Editor : 触发 change 事件
Editor->>Renderer : render()
Renderer->>Signal : 读取 segments/gaps
...
- [Signal.js:260-288](file://src/models/Signal.js#L260-L288)
...
## 结论
信号模型通过清晰的数据结构与完善的算法实现了高效的波形编辑能力。Signal 类提供了强大的段管理、时钟信号生成与分隔符支持；Segment 类保证了数据的有效性；Project 类提供了事件驱动的项目管理；SignalRenderer 与 colors.js 提供了高质量的可视化效果。配合 Exporter 与 StorageManager，实现了完整的序列化、版本兼容与多 sheet 管理。整体架构层次清晰、职责明确，易于扩展与维护。
...
- 信号 JSON 结构：包含 id、name、type、color、segments、clockConfig、gaps
...
- Signal 的段合并、时钟信号生成、边界移动、分隔符管理
- Project 的事件系统、时间轴转换、信号排序

**章节来源**
- [test-runner.html:57-304](file://tests/test-runner.html#L57-L304)